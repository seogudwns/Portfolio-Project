...
userAuthRouter.get("/userlist/:type/:pieceword", 
    login_required, 
    async (req, res, next) => {
    try {
        // 전체 사용자 목록을 얻음
        ...
        if (type === "user_name") {
            resultList = await userAuthService.getUsersWithRestrict({ pieceword });  //* 이름검색 완료
        } else if (type === "user_email") {
            resultList = await userAuthService.getUsersWithRestrict2({ pieceword });  //* 이메일검색 완료
